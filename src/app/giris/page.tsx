...
export default function LoginPage() {
  ...
  return (
    <div className="flex h-dvh flex-col bg-background text-foreground animated-gradient-bg">
      <header className="sticky top-0 z-10 flex h-16 shrink-0 items-center justify-between gap-4 border-b border-black/20 bg-transparent px-4 text-black">
        <Button variant="ghost" size="icon" onClick={() => router.back()} disabled={isSubmitting}>
          <ArrowLeft className="h-6 w-6" />
        </Button>
      </header>
      <main className="flex-1 flex flex-col justify-center items-center p-6 text-black">
        <div className="w-full max-w-md space-y-8">
          <div className="text-center">
            <h1 className="text-3xl font-bold">Tekrar Hoş Geldin!</h1>
            <p className="text-black/80">Hesabına giriş yaparak maceraya devam et.</p>
          </div>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <div className="relative">
                        ...
                        <Input type="email" placeholder={t.login.emailPlaceholder} className="pl-10 h-12 bg-white/20 border-black/30 placeholder:text-black/60 focus:ring-black/80" {...field} />
                      </div>
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <div className="relative">
                        ...
                        <Input type={showPassword ? 'text' : 'password'} placeholder={t.login.passwordPlaceholder} className="pl-10 pr-10 h-12 bg-white/20 border-black/30 placeholder:text-black/60 focus:ring-black/80" {...field} />
              ...
              />
              <Button type="submit" className="w-full h-12 rounded-full font-bold text-lg bg-black text-white hover:bg-gray-800" disabled={isSubmitting}>
           ...
           <p className="text-center text-sm text-black/80">
              {t.login.noAccount}{' '}
              <Link href="/kayit" className="font-semibold text-black hover:underline">
                {t.login.signupNow}
              </Link>
            </p>
        </div>
